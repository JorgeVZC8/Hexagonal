...
public class GlobalErrorControler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException exception, HttpHeaders headers, HttpStatusCode status, WebRequest request) {
        ...
        return this.handleExceptionInternal(exception, body, headers, HttpStatusCode.valueOf(400), request);

    }

    private ProblemDetail createProblemDetailWithInvalidParams(Exception e) {

        ProblemDetail result = ProblemDetail.forStatusAndDetail(HttpStatusCode.valueOf(400), "Error de validación");
        result.setTitle("Error de validación");
        result.setType(URI.create("http://www.openwebinars.net/error/validation-error"));
        result.setProperty("datetime", DateTimeFormatter
                .ofPattern("dd/MM/yyyy hh:mm:ss")
                .withZone(ZoneId.systemDefault())
                .format(Instant.now()));

        if (e instanceof MethodArgumentNotValidException ex1) {
            List<ApiValidationSubError> invalidParams = ex1.getAllErrors()
                    .stream()
            ...
            List<ApiValidationSubError> invalidParams = ex2.getConstraintViolations()
                    .stream()
                    .map(ApiValidationSubError::fromConstraintViolation)
                    .toList();
            result.setProperty("invalid-params", invalidParams);
    ...
    @Builder
    public record ApiValidationSubError(String object,
                                        String message,
